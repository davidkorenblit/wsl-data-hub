...
<div class="space-y-6 text-neutral-800 dark:text-neutral-200 leading-relaxed text-base sm:text-lg font-sans">
  ...
  <article class="rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#121215] p-5 sm:p-7 space-y-4 text-neutral-900 dark:text-neutral-100 text-base sm:text-lg leading-relaxed">
    ...
    <p>
      הרכש של <strong>מארי ארפס (Mary Earps)</strong> היה מתבקש הן מקצועית והן זהותית. הקהל האנגלי מת עליה. מספיק לחזור לאבל שעטף את הקהל כשהיא החליטה לא לקחת חלק ביורו האחרון עם הנבחרת ובכל חיפוש קצר של שמה בטוויטר, אם נחריג את השיח סביב הספר שלה והאמפתון, היא אהובה עד מאוד. ומקצועית, היא טובה. ממש טובה, ועם זכייה אחת ביורו בוומבלי, גם קצת ניסיון יש לה. קצת...
    </p>
    <p>
  ...
  <div class="rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#121215] p-4 sm:p-5 space-y-4">
    ...
    <p class="text-[11px] font-mono text-neutral-400 dark:text-neutral-500 text-left" dir="ltr">
      Source: FBref / Stathead Official Domestic League Player Comparison (Goalkeeping)
    </p>
  </div>

  <!-- SECTION 2: Center Back Upgrade - Mapi León vs Wassa Sangare -->
  <div class="border-t border-neutral-200 dark:border-neutral-800 pt-6 mt-8 space-y-6">
    <div class="border-b border-neutral-200 dark:border-neutral-800 pb-3">
      <div class="text-xs font-mono uppercase text-neutral-500 dark:text-neutral-400 font-bold tracking-wider mb-1">
        חלק ד' (המשך): עמדת הבלמית
      </div>
      <h2 class="text-2xl font-bold tracking-tight text-neutral-900 dark:text-white mb-1">
        עמדת הבלמית: מאפי ליאון (Mapi León) מול וואסה סנגרה (Wassa Sangare)
      </h2>
      <p class="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 font-sans">
        השוואה בין עוגן ההגנה שעזבה (סנגרה, 1,483 דק') לבין בלמת העל שהצטרפה מברצלונה.
      </p>
    </div>

    <!-- Narrative Context for Mapi -->
    <article class="rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#121215] p-5 sm:p-7 space-y-4 text-neutral-900 dark:text-neutral-100 text-base sm:text-lg leading-relaxed">
      <p>
        את הניתוח על <strong>מאפי ליאון (Mapi León)</strong> יש לחלק לשני חלקים ברורים:
      </p>
      <p>
        ראשית, נצא מנקודת הנחה שהיא מחליפה את <strong>וואסה סנגרה (Wassa Sangare)</strong>, שהייתה הבלמת ששיחקה את החלק הכי משמעותי של דקות המשחק בסגל שעבר (1,483 דקות) ולכן ההשוואה היא אליה. אם כי, בכנות, זה לא פייר – לא כלפיה וגם לא כלפי אלנה בניתוח הקודם.
      </p>
      <p>
        מאפי ליאון היא שחקנית הגנה שהערך המוסף העצום שלה הוא התקפי, ועל כן הניתוח נוגע הן לערך ההתקפי והן להגנתי:
      </p>
    ...
    <div class="rounded-md border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-[#121215] p-4 sm:p-5 space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-neutral-100 dark:border-neutral-800 pb-3">
        <div>
          <h3 class="text-lg font-bold text-neutral-900 dark:text-white">
            נתוני השוואה: מאפי ליאון מול וואסה סנגרה
          </h3>
          <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            השוואת ביצועים התקפיים, הגנתיים ומדדי הצלחה קבוצתית
          </p>
        </div>

        <!-- Toggle Tabs -->
        <div class="inline-flex rounded-md p-1 bg-neutral-100 dark:bg-neutral-800 text-xs font-medium self-start sm:self-auto border border-neutral-200 dark:border-neutral-700">
          <button id="btn-df-season" onclick="switchDfTab('season')" class="px-3 py-1.5 rounded transition-all bg-white dark:bg-neutral-900 text-neutral-900 dark:text-white shadow-sm font-bold flex items-center gap-1.5">
            <span>📅 עונת 2025/26</span>
          </button>
          <button id="btn-df-career" onclick="switchDfTab('career')" class="px-3 py-1.5 rounded transition-all text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white flex items-center gap-1.5">
            <span>🌐 כל הקריירה</span>
          </button>
        </div>
      </div>

      <!-- DF Table 1: 2025/26 Season -->
      <div id="df-season-view" class="space-y-2">
        <div class="overflow-x-auto rounded border border-neutral-200 dark:border-neutral-800" dir="ltr">
          <table class="w-full text-left border-collapse text-xs">
            <thead>
              <tr class="bg-neutral-50 dark:bg-[#18181b] border-b border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400 font-mono uppercase tracking-wider">
                ...
                <th class="py-2.5 px-2 text-center font-semibold">MP</th>
                <th class="py-2.5 px-2 text-right font-semibold">Min</th>
                <th class="py-2.5 px-2 text-right font-semibold text-emerald-600 dark:text-emerald-400">Gls</th>
                <th class="py-2.5 px-2 text-right font-semibold text-emerald-600 dark:text-emerald-400">Ast</th>
                <th class="py-2.5 px-2 text-right font-semibold text-emerald-600 dark:text-emerald-400">G+A/90</th>
                <th class="py-2.5 px-2 text-right font-semibold text-amber-600 dark:text-amber-400">Crosses</th>
                <th class="py-2.5 px-2 text-right font-semibold">TklW</th>
                <th class="py-2.5 px-2 text-right font-semibold">Int</th>
                <th class="py-2.5 px-2 text-right font-semibold">CrdY</th>
                <th class="py-2.5 px-2 text-right font-semibold">PPM</th>
                <th class="py-2.5 px-3 text-right font-semibold">+/- (on Pitch)</th>
              </tr>
            </thead>
            <tbody class="font-mono tabular-nums divide-y divide-neutral-100 dark:divide-neutral-800">
              {% assign df_season = site.data.comparisons.df_sangare_vs_mapi_leon_2025_26.players %}
              {% for p in df_season %}
              <tr class="hover:bg-neutral-50 dark:hover:bg-[#18181c] {% if p.player == 'Mapi León' %}bg-emerald-500/5 font-semibold{% endif %}">
                <td class="py-2 px-3 sticky left-0 {% if p.player == 'Mapi León' %}bg-emerald-50 dark:bg-[#0f241a] text-emerald-800 dark:text-emerald-300 font-bold{% else %}bg-white dark:bg-[#121215]{% endif %} border-r border-neutral-100 dark:border-neutral-800">
                  {{ p.player }}
                </td>
                <td class="py-2 px-2 text-neutral-500">{{ p.squad }}</td>
                <td class="py-2 px-2 text-center">{{ p.mp }}</td>
                <td class="py-2 px-2 text-right">{{ p.min }}</td>
                <td class="py-2 px-2 text-right font-bold text-emerald-600 dark:text-emerald-400">{{ p.gls }}</td>
                <td class="py-2 px-2 text-right font-bold text-emerald-600 dark:text-emerald-400">{{ p.ast }}</td>
                <td class="py-2 px-2 text-right font-bold text-emerald-600 dark:text-emerald-400">{{ p.g_plus_a90 }}</td>
                <td class="py-2 px-2 text-right font-bold text-amber-600 dark:text-amber-400">{{ p.crs }}</td>
                <td class="py-2 px-2 text-right">{{ p.tklw }}</td>
                <td class="py-2 px-2 text-right">{{ p.int }}</td>
                <td class="py-2 px-2 text-right">{{ p.crd_y }}</td>
                <td class="py-2 px-2 text-right font-semibold">{{ p.ppm }}</td>
                <td class="py-2 px-3 text-right font-bold {% if p.plus_minus > 0 %}text-emerald-600 dark:text-emerald-400{% else %}text-rose-600 dark:text-rose-400{% endif %}">{% if p.plus_minus > 0 %}+{% endif %}{{ p.plus_minus }}</td>
              </tr>
              {% endfor %}
            </tbody>
          </table>
      ...
      <!-- DF Table 2: Career -->
      <div id="df-career-view" class="hidden space-y-2">
        <div class="overflow-x-auto rounded border border-neutral-200 dark:border-neutral-800" dir="ltr">
          <table class="w-full text-left border-collapse text-xs">
            <thead>
              <tr class="bg-neutral-50 dark:bg-[#18181b] border-b border-neutral-200 dark:border-neutral-800 text-neutral-500 dark:text-neutral-400 font-mono uppercase tracking-wider">
            ...
            <tbody class="font-mono tabular-nums divide-y divide-neutral-100 dark:divide-neutral-800">
              {% assign df_career = site.data.comparisons.df_sangare_vs_mapi_leon_career.players %}
              {% for p in df_career %}
              <tr class="hover:bg-neutral-50 dark:hover:bg-[#18181c] {% if p.player == 'Mapi León' %}bg-emerald-500/5 font-semibold{% endif %}">
                <td class="py-2 px-3 sticky left-0 {% if p.player == 'Mapi León' %}bg-emerald-50 dark:bg-[#0f241a] text-emerald-800 dark:text-emerald-300 font-bold{% else %}bg-white dark:bg-[#121215]{% endif %} border-r border-neutral-100 dark:border-neutral-800">
                  {{ p.player }}
  ...
  <script>
    ...
    function switchDfTab(tab) {
      const seasonView = document.getElementById('df-season-view');
      const careerView = document.getElementById('df-career-view');
      const btnSeason = document.getElementById('btn-df-season');
      const btnCareer = document.getElementById('btn-df-career');
